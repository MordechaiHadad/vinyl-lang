...
pub fn format_range(source: &str, config: &FormatterConfig, range: std::ops::Range<usize>) -> Result<String, FormatError> {
...
pub fn format_project(source_root: &Path) -> Result<(), Vec<FormatError>> {
    let source_root =
        source_root.canonicalize().map_err(|e| vec![FormatError::Io(e)])?;
...
mod tests {
    ...
    fn configurable_indent_width() {
        ...
        assert!(result.contains("  return 1;"), "expected 2-space indent, got:\n{result}");
    }
