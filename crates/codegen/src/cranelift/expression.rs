...
use super::types::{element_byte_size, ir_type_from_primitive};
...
impl<'a> CodegenCtx<'a> {
    pub fn compile_expr(&mut self, expr: &HirExpression) -> Result<ir::Value, CraneliftError> {
        match &expr.kind {
            ...
            HirExpressionKind::Unit => Ok(self.func.builder.ins().iconst(types::I8, 0)),
            HirExpressionKind::Bool(b) => Ok(self.func.builder.ins().iconst(types::I8, *b as i64)),
            HirExpressionKind::Char(c) => Ok(self.func.builder.ins().iconst(types::I32, *c as i64)),
            HirExpressionKind::String(_) => Err(CraneliftError::Msg(
                "string expressions not supported in codegen yet".to_string(),
            )),
            HirExpressionKind::Ident(name) => self.read_var(name),
            ...
            HirExpressionKind::Call { function, args } => {
                if let HirExpressionKind::Ident(name) = &function.kind {
                    let callee_id = self
                        .module
                        .decls
                        .iter()
            ...
            HirExpressionKind::Tuple(elements, _) => {
                ...
                let total_size = crate::layout::size_of(tuple_type, ptr_size);
                let slot = self.func.builder.create_sized_stack_slot(StackSlotData::new(
                    StackSlotKind::ExplicitSlot,
                    total_size,
                    0,
                ...
                if let Type::Tuple(element_types) = tuple_type {
                    for (i, element) in elements.iter().enumerate() {
                        let val = self.compile_expr(element)?;
                        let offset =
                            crate::layout::tuple_field_offset(i, element_types, ptr_size);
            ...
            HirExpressionKind::FieldAccess { object, name, .. } => {
                ...
                let offset = match &object.type_ {
                    Type::Tuple(element_types) => {
                        ...
                        crate::layout::tuple_field_offset(index, element_types, ptr_size)
                    }
                    Type::Named(type_name) => match self.module.types.get(type_name) {
                        Some(HirItemKind::Struct(s)) => {
                            let field_types: Vec<(String, Type)> = s
                                ...
                                .collect();
                            let (_, field_layouts) =
                                crate::layout::struct_layout(&field_types, s.repr_c, ptr_size);
                            let field_idx = s
                                .fields
                                .iter()
                        ...
                        Some(HirItemKind::TupleStruct(t)) => {
                            ...
                            crate::layout::tuple_field_offset(index, &t.types, ptr_size)
                        }
                        _ => {
    ...
    fn compile_enum_variant(
        ...
        let (total_size, data_offset, _disc_size) =
            crate::layout::enum_layout(std::slice::from_ref(&payload_types), ptr_size);
        if total_size > 8 {
            return Err(CraneliftError::Msg(format!(
        ...
        for (i, elem) in payload.iter().enumerate() {
            ...
            let elem_align = crate::layout::align_of(&payload_types[i], ptr_size);
            ...
            data_offset_acc += crate::layout::size_of(&payload_types[i], ptr_size);
        }
