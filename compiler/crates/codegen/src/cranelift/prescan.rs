...
fn prescan_stmts(stmts: &[HirStatement], refed: &mut HashSet<String>) {
    for stmt in stmts {
        match &stmt.kind {
            ...
            HirStatementKind::Expr(e, _) | HirStatementKind::Value(e, _) => prescan_expr(e, refed),
            HirStatementKind::Return(Some(e), _) => prescan_expr(e, refed),
            HirStatementKind::Return(None, _) => {}
            HirStatementKind::Loop { body, .. } => prescan_stmts(body, refed),
            HirStatementKind::Break(_) | HirStatementKind::Continue(_) => {}
            HirStatementKind::Assign { target, value, .. } => {
                if let HirAssignTarget::Deref(e, _) = target {
...
fn prescan_expr(expr: &HirExpression, refed: &mut HashSet<String>) {
    match &expr.kind {
        HirExpressionKind::Ident(_name, _) => {}
        ...
        HirExpressionKind::Block(stmts, _) => prescan_stmts(stmts, refed),
