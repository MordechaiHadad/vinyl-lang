...
fn script_mode_loads_imported_module_for_missing_import_suggestion() {
    ...
    // open only main.vn; math.vn exists on disk but is never opened directly
    lsp.send(json!({
        "jsonrpc": "2.0", "method": "textDocument/didOpen",
        "params": {
            "textDocument": { "uri": main_uri, "languageId": "vinyl", "version": 1,
    ...
    // remove the import: the missing-import suggestion should surface because
    // math.vn was loaded into the vfs when main.vn was opened
    lsp.send(json!({
        "jsonrpc": "2.0", "method": "textDocument/didChange",
        "params": {
            "textDocument": { "uri": main_uri, "version": 2 },
    ...
    let diag = loop {
        let notification = lsp.notification("textDocument/publishDiagnostics");
    ...
    let messages: Vec<String> = diag["params"]["diagnostics"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert!(
        messages
            .iter()
            .any(|m| m.contains("missing import") && m.contains("math::double")),
        "removing the import should surface the missing-import suggestion, got: {messages:?}"
    );
    ...
    let actions = lsp.response(2)["result"].as_array().unwrap().clone();
    assert!(
        actions
            .iter()
            .any(|action| action["title"] == "Add import `parent::math`"),
        "qualified reference should offer its module import, got: {actions:?}"
    );
}
...
#[test]
fn completion_field_access_strict_only_fields() {
    let project = TestProject::new();
    std::fs::write(
        &project.main,
    ...
    for expected in ["x", "y"] {
        assert!(
            items.iter().any(|i| i["label"] == expected),
            "field `{expected}` should appear after `.`, got: {items:#?}"
        );
    }
    for forbidden in ["double", "p", "Point", "main", "n"] {
        assert!(
            !items.iter().any(|i| i["label"] == forbidden),
...
#[test]
fn completion_field_access_strict_with_typed_prefix() {
    let project = TestProject::new();
    std::fs::write(
        &project.main,
        "struct Point { public x: int32, public y: int32 }\nfn double(n: int32): int32 { n * 2 }\nfn main() {\n    let p = Point { x: 1, y: 2 };\n    p.d\n}\n",
    ...
    for forbidden in ["double", "p", "Point", "main", "n"] {
        assert!(
            !items.iter().any(|i| i["label"] == forbidden),
            "`{forbidden}` should not appear when typing a field prefix, got: {items:#?}"
        );
    }
...
#[test]
fn completion_field_access_strict_on_non_struct() {
    let project = TestProject::new();
    std::fs::write(
        &project.main,
        "fn main() {\n    let n: int32 = 1;\n    n.\n}\n",
    ...
    assert!(
        items.is_empty(),
        "expected no completions for a non-struct receiver, got: {items:#?}"
    );
...
#[test]
fn completion_field_access_after_dot_with_trailing_pipe() {
    let project = TestProject::new();
    std::fs::write(
        &project.main,
        "struct Point { public x: int32, public y: int32 }\nfn double(n: int32): int32 { n * 2 }\nfn main() {\n    let p = Point { x: 1, y: 2 };\n    p. |> double()\n}\n",
    ...
    for expected in ["x", "y"] {
        assert!(
            items.iter().any(|i| i["label"] == expected),
            "field `{expected}` should appear after `.` with a trailing pipe, got: {items:#?}"
        );
    }
